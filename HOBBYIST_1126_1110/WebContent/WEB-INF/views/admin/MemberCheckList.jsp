...
<style>
	...
	#toFirstBtn1, #beforeBtn1, #afterBtn1, #toLastBtn1 {
		border-radius: 50%;
	}
</style>
<meta charset="UTF-8">
...
<!-- 													</td> -->
					...
					<nav aria-label="Page navigation example">
						<ul class="pagination justify-content-center">
							<li class="page-item">
								<button class="page-link" id="toFirstBtn1" onclick="location.href='<%= request.getContextPath() %>/memberCheck.admin?currentPage=1'">&laquo;</button>
							</li>
						<!-- prev -->
							<li class="page-item">
								<button class="page-link" id="beforeBtn1" onclick="<%= request.getContextPath() %>/memberCheck.admin?currentPage=<%= pi.getCurrentPage() -1 %>'" aria-label="Previous">&laquo;</button>
							</li>
						...
						<!-- 페이지넘버 -->
							<% for (int p = pi.getStartPage(); p <= pi.getEndPage(); p++) { %>
								...
								<%      } else { %> 
											<li class="page-item" id="numBtn1"><a class="page-link" href="<%= request.getContextPath() %>/memberCheck.admin?currentPage=<%= p %>"><%= p %></a></li>
									<% } %>
							<% 	} %>
							
							<!-- next -->
							<li class="page-item">
								<button id="afterBtn1" class="page-link" onclick="location.href='<%= request.getContextPath() %>/memberCheck.admin?currentPage=<%= pi.getCurrentPage() + 1 %>'"
									aria-label="Next"> &raquo;</button>
							</li>
							<li class="page-item">
								<button id="toLastBtn1" class="page-link" onclick="location.href='<%= request.getContextPath() %>/memberCheck.admin?currentPage=<%= pi.getMaxPage() %>'" 
									aria-label="Next"> &raquo; </button>
							</li>
						</ul>
					...
					<div class="col-md-10">
						<div class="card">
							<div class="card-header">
								<div class="card-title"><span style="color: #6CAB76">튜터(TUTOR)</span> 리스트</div>
							</div>
							<div class="card-body">
							<table class="table table-hover">
								<form>	
											...
											<%	for(int i = 0; i < tutorList.size(); i++){ %>
												<tr>
													...
													<td>
														...
														<input type="hidden" id="tuteePwd" name="tuteePwd" value="<%= tutorList.get(i).getMemberPwd() %>">
													</td>
													<td>
														<%= tutorList.get(i).getMemberEnrollDate() %>
														<input type="hidden" id="tuteeEnrollDate" name="tuteeEnrollDate" value="<%= tutorList.get(i).getMemberEnrollDate() %>">
													</td>
													<td>
					...
					<nav aria-label="Page navigation example">
						<ul class="pagination justify-content-center">
							<li class="page-item">
								<button class="page-link" id="toFirstBtn2" onclick="location.href='<%= request.getContextPath() %>/memberCheck.admin?currentPage=1'">&laquo;</button>
							</li>
						<!-- prev -->
							<li class="page-item">
								<button class="page-link" id="beforeBtn2" onclick="<%= request.getContextPath() %>/memberCheck.admin?currentPage=<%= pi.getCurrentPage() -1 %>'" aria-label="Previous">&laquo;</button>
							</li>
						...
						<!-- 페이지넘버 -->
							<% for (int p = pi.getStartPage(); p <= pi.getEndPage(); p++) { %>
								<% 		if(p == pi.getCurrentPage()) { %>
									<li class="page-item" id="choosen"><a class="page-link" href=""><%= p %></a></li> <!-- 현재 페이지는 선택 못하게 -->
								<%      } else { %> 
											<li class="page-item" id="numBtn2"><a class="page-link" href="<%= request.getContextPath() %>/memberCheck.admin?currentPage=<%= p %>"><%= p %></a></li>
									<% } %>
							<% 	} %>
							
							<!-- next -->
							<li class="page-item">
								<button id="afterBtn2" class="page-link" onclick="location.href='<%= request.getContextPath() %>/memberCheck.admin?currentPage=<%= pi.getCurrentPage() + 1 %>'"
									aria-label="Next"> &raquo;</button>
							</li>
							<li class="page-item">
								<button id="toLastBtn2" class="page-link" onclick="location.href='<%= request.getContextPath() %>/memberCheck.admin?currentPage=<%= pi.getMaxPage() %>'" 
									aria-label="Next"> &raquo; </button>
							</li>
						</ul>
