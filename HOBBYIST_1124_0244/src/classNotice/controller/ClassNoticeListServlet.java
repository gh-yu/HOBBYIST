...
import classNotice.model.vo.PageInfo;
...
public class ClassNoticeListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		ClassNoticeService cService = new ClassNoticeService(); // 여러 번 불러야 해서 service객체 만들어 놓기
		...
		if (request.getParameter("currentPage") != null) {
			...
			// currentPage가 null이다 -> 처음 페이지에 들어왔다 -> currentPage = 1 (null이 아닐때만
			// currentPage값 바꿔주면 됨)
		...
		// int값 두 개를 나누면 소수점까지 안 나오기 때문에 double로 형 변환 필요 -> Math.ceil메소드로 올림처리 후 int로 형
		// 변환
